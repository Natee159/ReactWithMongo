...
function Central2() {
  ...
  useEffect(() => {
    axios.get("http://192.168.99.100:8080/api/users/showcentral2").then(res => {
  ...
  const codeCentral2 = (fx,d, h, x) => {
    let func = (X) => parse(fx).evaluate({ x: X });
    var ans = 0
    var e = 0
    var real = 0
    if (d === 1) {
        ans = (-func(x+(2*h) ) +8 * func(x+h) -8 * func(x-h) +func(x-(2*h) )) / (12*h)
        console.log(ans)
        real = funcd(x)
      } else if (d === 2) {
        ans = (-func(x+(2*h) ) +16 * func(x+h) - 30 *func(x) +16 * func(x-h) -func(x-(2*h) )) / (Math.pow(h, 2)*12)
        real = funcd2(x)
      } else if (d === 3) {
        ans = (-func(x + (3 * h)) + 8 * func(x + (2 * h)) - 13 * func(x +h) + 13 * func(x -h) - 8 * func(x-(2*h)) +func(x-(3*h))) / (Math.pow(h, 3)*8)
        real = funcd3(x)
      } else if (d === 4) {
        ans = (-func(x + (3 * h)) + 12 * func(x + (2 * h)) - 39 * func(x +h) + 56 * func(x) - 39 * func(x -h) +12* func(x-(2*h)) -func(x-(3*h))) / (Math.pow(h, 4)*6)
        real = funcd4(x)
      }
    console.log("cal :", ans)
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <div style={{ padding: 24, background: '#F8CEA5', minHeight: 30 }}><h1>Central2 Divide Difference</h1></div>
      <Row>
        <Col span={8}>
