...
function Backward() {
  ...
  useEffect(() => {
    axios.get("http://192.168.99.100:8080/api/users/showbackward").then(res => {
  ...
  const codebackward = (fx,d, h, x) => {
    let func = (X) => parse(fx).evaluate({ x: X });
    var ans = 0
    var e = 0
    var real = 0
    if (d === 1) {
      ans = (func(x) - func(x-(h))) / h
      console.log(ans)
      real = funcd(x)
    } else if (d === 2) {
      ans = (func(x) - 2 * func(x - h) + func(x-(2*h))) / Math.pow(h, 2)
      real = funcd2(x)
    } else if (d === 3) {
      ans = (func(x) - 3 * func(x - h) + 3 * func(x-(2*h)) - func(x-(3*h))) / Math.pow(h, 3)
      real = funcd3(x)
    } else if (d === 4) {
      ans = (func(x) - 4 * func(x - h) + 6 * func(x-(2*h)) -4 * func(x-(3*h))+ func(x-(4*h))) / Math.pow(h, 4)
      real = funcd4(x)
    }
    console.log("cal :", ans)
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <div style={{ padding: 24, background: '#F8CEA5', minHeight: 30 }}><h1>Backward Divide Difference</h1></div>
      <Row>
        <Col span={8}>
