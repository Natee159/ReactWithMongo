...
function FalsePosition() {
  ...
  const [fx, setfx] = useState("(1/x)-43")
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <div style={{ padding: 24, background : '#F8CEA5', minHeight: 30 }}><h1>FalsePosition.</h1></div>
      ...
      <Row>
        <Col span={8}>
          <div><h1>xl</h1>
            <InputNumber min={0} max={10} defaultValue={0.02} placeholder="xl" onChange={value => setxl(value)} />
      ...
      <Row>
        <Col span={8}>
          <div><h1>xr</h1>
            <InputNumber min={0} max={10} defaultValue={0.03} placeholder="xr" onChange={value => setxr(value)} />
            <span className='input-group-text'></span>
          </div>
        </Col>
        <Col span={8}>
      ...
      <div>
        <Button onClick={codefalsePosition} type="primary">input</Button>
        {x}
