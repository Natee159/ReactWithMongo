...
function Backward2() {
    ...
    useEffect(() => {
      axios.get("http://192.168.99.100:8080/api/users/showbackward2").then(res => {
    ...
    const codeBackward2 = (fx, d, h, x) => {
        let func = (X) => parse(fx).evaluate({ x: X });
        var ans = 0
        var e = 0
        var real = 0
        if (d === 1) {
            ans = (3 * func(x) - 4 * func(x - h) + func(x - (2 * h))) / (2 * h)
            console.log(ans)
            real = funcd(x)
        } else if (d === 2) {
            ans = (2 * func(x) - 5 * func(x - h) + 4 * func(x - (2 * h)) - func(x - (3 * h))) / Math.pow(h, 2)
            real = funcd2(x)
        } else if (d === 3) {
            ans = (5 * func(x) - 18 * func(x - h) + 24 * func(x - (2 * h)) - 14 * func(x - (3 * h)) + 3 * func(x - (4 * h))) / (Math.pow(h, 3) * 2)
            real = funcd3(x)
        } else if (d === 4) {
            ans = (3 * func(x) - 14 * func(x - h) + 26 * func(x - (2 * h)) - 24 * func(x - (3 * h)) + 11 * func(x - (4 * h)) - 2 * func(x - (5 * h))) / Math.pow(h, 4)
            real = funcd4(x)
        }
        console.log("cal :", ans)
    ...
    return (
        <Content style={{ margin: '0 16px' }}>
            <div style={{ padding: 24, background: '#F8CEA5', minHeight: 30 }}><h1>Backward2 Divide Difference</h1></div>
            <Row>
                <Col span={8}>
