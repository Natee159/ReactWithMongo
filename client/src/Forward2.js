...
function Forward2() {
  ...
  const codeforward2 = (fx,d, h, x) => {
    let func = (X) => parse(fx).evaluate({ x: X });
    var ans = 0
    var e = 0
    var real = 0
    if (d === 1) {
      ans = (-func(x+(2*h) ) +4 * func(x+h) -3 * func(x)) / (2*h)
      console.log(ans)
      real = funcd(x)
    } else if (d === 2) {
      ans = (-func(x + (3 * h)) + 4 * func(x +(2*h)) - 5 * func(x+h)+ 2 * func(x)) / Math.pow(h, 2)
      real = funcd2(x)
    } else if (d === 3) {
      ans = (-3 * func(x + (4 * h)) + 14 * func(x + (3 * h)) - 24 * func(x +(2*h) ) + 18 * func(x+h) -5* func(x)) / (Math.pow(h, 3)*2)
      real = funcd3(x)
    } else if (d === 4) {
      ans = (-2 *func(x + (5 * h)) + 11 * func(x + (4 * h)) - 24 * func(x +(3*h) ) + 26 * func(x +(2*h) ) -14* func(x+h) +3* func(x)) / Math.pow(h, 4)
      real = funcd4(x)
    }
    console.log("cal :", ans)
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <div style={{ padding: 24, background: '#F8CEA5', minHeight: 30 }}><h1>Forward H2 Divide Difference</h1></div>
      <Row>
        <Col span={8}>
