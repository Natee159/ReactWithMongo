...
function Forward() {
  ...
  const codeforward = (fx,d, h, x) => {
    ...
    if (d === 1) {
      ...
      ans = (func(x + (2 * h)) - 2 * func(x + h) + func(x)) / Math.pow(h, 2)
      real = funcd2(x)
    } else if (d === 3) {
      ans = (func(x + (3 * h)) - 3 * func(x + (2 * h)) + 3 * func(x + h) - func(x)) / Math.pow(h, 3)
      real = funcd3(x)
    } else if (d === 4) {
      ans = (func(x + (4 * h)) - 4 * func(x + (3 * h)) + 6 * func(x + (2 * h)) - 4 * func(x + h) + func(x)) / Math.pow(h, 4)
      real = funcd4(x)
    }
    console.log("cal :", ans)
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <div style={{ padding: 24, background: '#F8CEA5', minHeight: 30 }}><h1>Forward Divide Difference</h1></div>
      <Row>
        <Col span={8}>
